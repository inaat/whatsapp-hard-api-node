...
class WhatsAppInstance {
    socketConfig = {
        ...
        getMessage: (key) => {
            return (
                dados.loadMessage(key.remoteJid, key.id)?.message || undefined
            )
        },
        ...
        if (cachedResult) {
            ...
            try {
                ...
                if (result.exists) {
                    ...
                    throw new Error(
                        'O número:' + id + ' não é um Whatsapp Valido'
                    )
                }
            } catch (error) {
    ...
    async assertSession(lid) {
        try {
            ...
            const assert = await this.instance.sock?.assertSessions(
                senderKeyJids
            )
    ...
    async setStatus(status, to, type, pause = false) {
        try {
            ...
            throw new Error(
                'Falha ao enviar a presença, verifique o id e tente novamente'
            )
        }
    }
    ...
    async mystatus(status) {
        try {
            ...
            return {
                error: true,
                message: 'Não foi possível alterar para o status ' + status,
            }
        }
    }
    ...
    async joinURL(url) {
        try {
            ...
            return {
                error: true,
                message:
                    'Erro ao entrar via URL, verifique se a url ainda é valida ou se o grupo é um grupo aberto.',
            }
        }
    }
    ...
    async groupSettingUpdate(id, action) {
        try {
            ...
            return {
                error: false,
                message: 'Alteração referente a ' + action + ' Concluida',
            }
        } catch (e) {
            //console.log(e)
            return {
                ...
                message:
                    'Erro ao alterar' +
                    action +
                    ' Verifique se você tem permissão ou se o grupo existe',
            }
        }
    }
    ...
    async groupUpdateSubject(id, subject) {
        try {
            ...
            return {
                error: false,
                message: 'Nome do grupo alterado para ' + subject,
            }
        } catch (e) {
            //console.log(e)
            return {
    ...
    async groupUpdateDescription(id, description) {
        try {
            ...
            return {
                error: false,
                message: 'Descrição do grupo alterada para ' + description,
            }
        } catch (e) {
            return {
                error: true,
